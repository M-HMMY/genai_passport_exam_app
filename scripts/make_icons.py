...
# ---------------------------------------------------------------- 見た目の設定
# 背景は左上から右下へのグラデーション。生成 AI らしさを出すため紫系にした。
BG_FROM = (76, 29, 149)      # 濃い紫
BG_TO = (168, 85, 247)       # 藤色
FG = (255, 255, 255)         # 白い字
...
# 「AI」の字形。一辺を 1 とした座標で書く。
TOP, BOTTOM = 0.235, 0.585        # 字の上端・下端
A_APEX = 0.44                     # A の頂点の x
A_LEFT, A_RIGHT = 0.3375, 0.5425  # A の左脚・右脚の、下端での中心 x
A_HALF = 0.0285                   # 脚の太さの半分（横方向）
BAR_Y = (0.445, 0.500)            # A の横棒
I_X = (0.635, 0.690)              # I の縦棒
...
def _leg_center(y, bottom_x):
    """A の脚の中心 x。頂点から下端へ、直線で開いていく。"""
    t = (y - TOP) / (BOTTOM - TOP)
    return A_APEX + t * (bottom_x - A_APEX)
...
def inside_glyph(x, y):
    ...
    # --- A（2 本の脚と横棒） ---
    left = _leg_center(y, A_LEFT)
    right = _leg_center(y, A_RIGHT)
    if abs(x - left) <= A_HALF or abs(x - right) <= A_HALF:
        return True
    if BAR_Y[0] <= y <= BAR_Y[1] and left <= x <= right:
        return True
