...
// ---- 問題文が本番で読み切れる長さか ----
// 本番は 60 問 / 60 分で 1 問あたりちょうど 60 秒しかない。
// 読むだけで時間を使い切る問題は、練習として不誠実になる。
for (const q of QUESTIONS) {
  const len = q.question.replace(/\s/g, '').length;
  if (len > 160) err(`問題 ${q.id}: 問題文が ${len} 字（本番は 1 問 60 秒。120 字までを目安に切り詰める）`);
  else if (len > 120) warn(`問題 ${q.id}: 問題文が ${len} 字とやや長い（1 問 60 秒で読める長さに）`);
}
...
{
  const OWN_PREFIX = 'genai-passport-app-';
  try {
