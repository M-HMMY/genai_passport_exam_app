...
export const cChatgptQuestions: Question[] = [
  ...
  },
  {
    ...
    answer: 1,
    explanation:
      'イが正しい。事前学習によって言葉の並び方を身につけ、それを使って次に来そうな語を選びます。アは誤りで、利用のたびにモデルが学習し直すわけではありません。ウは事実確認の話で、生成 AI に事実を調べる仕組みは入っていません。エは回答の登録の話であり、事前学習とは異なります。',
    level: 2,
  },
  {
    id: 'c-chatgpt-q04',
  ...
  },
  {
    ...
    choices: [
      ...
      '利用者の質問が長すぎると、途中の内容が失われてしまうため',
    ],
    answer: 1,
    explanation:
      'イが正しい。情報が足りなくても、言葉として自然な続きを組み立てることがあります。実在しない本や判例にもそれらしい説明を作ることがあるため、重要な事実は人が一次資料で確かめます。アは「わざとの嘘」という誤解です。ウは学習データがすべて誤りという極端な記述で、事実と異なります。エは質問の長さの話であり、原因の説明にはなりません。',
    level: 2,
  },
  ...
  },
  {
    ...
    choices: [
      'パラメータが多いほど複雑な規則性を持てる余地は増えるが、多いほど必ず賢いわけではない',
      'パラメータが多いモデルは、すべての用途で必ず高い精度を示す',
      'パラメータの数は、モデルの応答速度だけを決める値である',
      'パラメータの数は、学習に使ったデータの件数と必ず一致する',
    ],
    answer: 0,
    explanation:
      'アが正しい。データセットの量と質、学習方法、利用時の道具も性能を左右します。イは「必ず」と言い切った誤りで、この試験では大きい方が優れていると断じる選択肢を疑ってください。ウは応答速度だけに限定した誤りです。エはパラメータ数とデータ件数を混同した記述であり、両者は別のものです。',
    level: 2,
  },
  ...
  },
  {
    ...
    choices: [
      'Microsoft 365 の各アプリに組み込まれ、文書作成や集計を支援する',
      '文章・画像・音声・動画・コードなど複数種類の情報を扱い、Google のサービスと組み合わされる',
      '対話の履歴を保存せず、一問一答の用途だけに限定されている',
      'プログラムの作成だけを目的とし、文章の作成には対応していない',
    ],
    answer: 1,
    explanation:
      'イが正しい。2023 年に最初の世代が発表され、当初からマルチモーダルを前面に出しました。検索や Gmail、Google ドライブなどとの組み合わせも特徴です。アは Copilot の説明です。ウは用途を限定した誤りです。エも誤りで、文章の作成にも対応しています。',
    level: 2,
  },
  {
    id: 'c-chatgpt-q23',
    ...
    choices: [
      '検索エンジンの結果を並べ替えることだけを行う',
      '画像の生成に特化しており、文章は扱わない',
      '文章や画像、アップロードした長い文書を読み、要約・分析・文章作成などを行う',
      '表計算ソフトの中だけで動作し、単独では利用できない',
    ],
    answer: 2,
    explanation:
  ...
  },
  {
    ...
    choices: [
      '公開時期が新しいものほど、あらゆる用途で優れていると考えてよい',
      'それぞれ専用の分野が決まっており、他の用途にはまったく使えない',
      'パラメータ数が最も多いものを選べば、どの業務でも失敗しない',
      '機能は重なり更新で広がるため、優劣を言い切らず提供元と使われ方で捉える',
    ],
    answer: 3,
    explanation:
      'エが正しい。Gemini も文章やコードを作り、Claude も画像を読み、Copilot もチャットで答えます。同じ名前でも版や組み込まれた製品によって機能が違います。アは公開時期だけで優劣を決める誤りです。イは専用分野があるという言い切りで、事実と異なります。ウはパラメータ数だけで判断する誤りです。',
    level: 2,
  },
];
